function initBeautyAddictLogin() {
  ...
  if (!window.firebase) {
    console.log("Firebase absent");
    window.__baaLoginInitialized = false;
    return;
  }
  ...
  auth.onAuthStateChanged(async (user) => {
    if (!user) {
      ...
      document.getElementById("login-tab").onclick = function() {
        ...
        document.getElementById("login-tab").className = "baa-tab baa-active";
        document.getElementById("signup-tab").className = "baa-tab baa-inactive";
        document.getElementById("baa-message").innerHTML = "";
      };

      document.getElementById("signup-tab").onclick = function() {
        document.getElementById("login-view").style.display = "none";
        document.getElementById("signup-view").style.display = "block";
        document.getElementById("signup-tab").className = "baa-tab baa-active";
        document.getElementById("login-tab").className = "baa-tab baa-inactive";
        document.getElementById("baa-message").innerHTML = "";
      };
      ...
      document.getElementById("login-btn").onclick = async function() {
        ...
        try {
          ...
          document.getElementById("baa-message").innerHTML = "Email ou mot de passe incorrect.";
        }
      };
      ...
      document.getElementById("signup-btn").onclick = async function() {
        ...
        try {
          ...
          await db.collection("users").doc(userCred.user.uid).set({
            prenom: firstname, nom: lastname, email: email,
            accountStatus: "pending", role: "member",
            moduleActuel: 1, progression: 0, createdAt: new Date()
          });
          msg.innerHTML = "Compte cree ! En attente de validation.";
          emailjs.init("D_JtKhPDgOQWi_ECO");
          emailjs.send("service_wr9mlhk", "template_qffpmql", {
            prenom: firstname, nom: lastname, email: email,
            date: new Date().toLocaleDateString("fr-FR")
          }).catch(function(err) { console.log("EmailJS erreur:", err); });
        } catch (e) {
          document.getElementById("baa-message").innerHTML = e.message;
        }
      };
    ...
    if (data && data.accountStatus === "pending") {
      const existing = document.getElementById("baa-login-overlay");
      if (existing) existing.remove();
      if (!document.getElementById("baa-pending-msg")) {
        const msg = document.createElement("div");
        msg.id = "baa-pending-msg";
        ...
        msg.innerHTML = "<div style='font-size:60px;margin-bottom:20px;'>🐦</div><h2 style='color:#8b735d;'>Compte en attente de validation</h2><p style='color:#666;max-width:400px;'>Ton inscription a bien ete recue. Tu recevras un email des que ton compte sera active.</p><button onclick=\"firebase.auth().signOut()\" style='margin-top:30px;background:#f3e7d3;color:#8a6a35;border:1px solid #c8a96b;padding:10px 20px;border-radius:10px;cursor:pointer;font-size:14px;'>Se deconnecter</button>";
    ...
    if (data && data.accountStatus === "active") {
      ...
      if (!document.querySelector("#baa-menu-btn")) {
        ...
        db.collection("users").doc(auth.currentUser.uid).get().then(function(snap) {
          const d = snap.data();
          if (d.photoURL) {
            menuBtn.innerHTML = "<img src='" + d.photoURL + "' style='width:100%;height:100%;object-fit:cover;' />";
          } else {
            menuBtn.innerHTML = "<span style='color:#8a6a35;font-size:16px;font-weight:bold;font-family:Arial;'>" + (d.prenom ? d.prenom[0].toUpperCase() : "") + (d.nom ? d.nom[0].toUpperCase() : "") + "</span>";
          }
        });

        menuBtn.onclick = function(e) {
          e.stopPropagation();
          const existing = document.getElementById("baa-side-menu");
          if (existing) { existing.remove(); return; }

          const menu = document.createElement("div");
          menu.id = "baa-side-menu";
          ...
          db.collection("users").doc(auth.currentUser.uid).get().then(function(snap) {
            ...
            var avatarHTML = d.photoURL
              ? "<img src='" + d.photoURL + "' style='width:70px;height:70px;border-radius:50%;object-fit:cover;border:3px solid #e8d4b0;' />"
              : "<div style='width:70px;height:70px;border-radius:50%;background:#c9a86a;display:flex;align-items:center;justify-content:center;border:3px solid #e8d4b0;'><span style='color:white;font-size:24px;font-weight:bold;'>" + (d.prenom ? d.prenom[0].toUpperCase() : "") + (d.nom ? d.nom[0].toUpperCase() : "") + "</span></div>";

            var adminItem = data.role === "admin"
              ? "<div class='baa-menu-item' id='menu-admin' style='display:flex;align-items:center;gap:14px;padding:16px 20px;cursor:pointer;border-bottom:1px solid #f0e6d3;'><span style='font-size:20px;'>⚙️</span><span style='color:#8a6a35;font-size:15px;font-weight:600;'>Panneau Admin</span></div>"
              : "";
            ...
            document.querySelectorAll(".baa-menu-item").forEach(function(item) {
              item.onmouseenter = function() { item.style.background = "#f0e6d3"; };
              item.onmouseleave = function() { item.style.background = "transparent"; };
            });

            if (data.role === "admin") {
              document.getElementById("menu-admin").onclick = function() {
                menu.remove();
                openAdminPanel();
              };
            }

            document.getElementById("menu-moncompte").onclick = function() {
              menu.remove();
              openInfoPanel();
            };
            document.getElementById("menu-carnet").onclick = function() {
              menu.remove();
              openCarnetPanel();
            };
            document.getElementById("menu-commandes").onclick = function() {
              menu.remove();
              openCommandesPanel();
            };
            document.getElementById("menu-notes").onclick = function() {
              menu.remove();
              openNotesPanel();
            };
            document.getElementById("menu-outils").onclick = function() {
              menu.remove();
              openOutilsPanel();
            };
          });
              ...
              document.getElementById("accept-" + uid).onclick = function() {
                db.collection("users").doc(uid).update({ accountStatus: "active" }).then(function() {
                  emailjs.send("service_wr9mlhk", "template_wk2j4mg", {
                    ...
                    date: new Date().toLocaleDateString("fr-FR")
                  }).then(function() { console.log("Email bienvenue envoye"); }).catch(function(err) { console.log(err); });
                  document.getElementById("row-" + uid).remove();
                  if (document.getElementById("admin-members-list").children.length === 0) {
                    document.getElementById("admin-members-list").innerHTML = "<p style='color:#999;'>Aucun membre en attente.</p>";
                  }
                });
              };
        ...
        function loadAllMembers() {
          ...
          db.collection("users").get().then(function(snapshot) {
            ...
            snapshot.forEach(function(docSnap) {
              ...
              list.appendChild(row);
              document.getElementById("block-" + uid).onclick = function() {
                const newStatus = d.accountStatus === "suspended" ? "active" : "suspended";
                db.collection("users").doc(uid).update({ accountStatus: newStatus }).then(function() { loadAllMembers(); });
              };
              document.getElementById("delete-" + uid).onclick = function() {
                if (confirm("Supprimer definitivement " + d.prenom + " " + d.nom + " ?")) {
                  db.collection("users").doc(uid).delete().then(function() {
                    document.getElementById("row-all-" + uid).remove();
                  });
                }
              };
            });
        ...
        function loadDashboard() {
          ...
          db.collection("users").where("accountStatus", "==", "active").get().then(function(snapshot) {
            ...
            snapshot.forEach(function(docSnap) {
              ...
              const objectif = d.suiviObjectif || 0;
              const realise = d.suiviRealise || 0;
              const pct = objectif > 0 ? Math.min(100, Math.round(realise / objectif * 100)) : 0;
              const taux = realise >= 100 ? 30 : 20;
              const commission = (realise * taux / 100).toFixed(2);
...
<div style='color:#888;font-size:12px;margin-top:2px;'>Derniere connexion : " + derniereConnexion + "</div></div><span style='background:" + checklistColor + ";color:white;padding:4px 10px;border-radius:10px;font-size:12px;font-weight:bold;'>" + checklistText + "</span></div><div style='display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px;margin-bottom:10px;'><div style='text-align:center;padding:8px;background:#f8f3ee;border-radius:8px;'><div style='color:#888;font-size:11px;'>Objectif</div><div style='color:#3a3a3a;font-size:14px;font-weight:bold;'>" + (objectif || "-") + " euros</div></div><div style='text-align:center;padding:8px;background:#f8f3ee;border-radius:8px;'><div style='color:#888;font-size:11px;'>CA realise</div><div style='color:#3a3a3a;font-size:14px;font-weight:bold;'>" + (realise || "-") + " euros</div></div><div style='text-align:center;padding:8px;background:#f8f3ee;border-radius:8px;'><div style='color:#888;font-size:11px;'>Commission</div><div style='color:#c9a86a;font-size:14px;font-weight:bold;'>" + commission + " euros</div></div></div><div style='background:#f0e6d3;border-radius:20px;height:8px;overflow:hidden;'><div style='background:#c9a86a;height:100%;border-radius:20px;width:" + pct + "%;'></div></div><div style='text-align:right;color:#888;font-size:11px;margin-top:4px;'>" + pct + "% de l objectif</div>";
        ...
        document.getElementById("tab-pending").onclick = function() {
          document.getElementById("tab-pending").style.background = "#c9a86a";
          document.getElementById("tab-pending").style.color = "white";
          document.getElementById("tab-pending").style.border = "none";
          document.getElementById("tab-all").style.background = "#f3e7d3";
          document.getElementById("tab-all").style.color = "#8a6a35";
          document.getElementById("tab-all").style.border = "1px solid #c8a96b";
          document.getElementById("tab-dashboard").style.background = "#f3e7d3";
          document.getElementById("tab-dashboard").style.color = "#8a6a35";
          document.getElementById("tab-dashboard").style.border = "1px solid #c8a96b";
          loadPending();
        };
        document.getElementById("tab-all").onclick = function() {
          document.getElementById("tab-all").style.background = "#c9a86a";
          document.getElementById("tab-all").style.color = "white";
          document.getElementById("tab-all").style.border = "none";
          document.getElementById("tab-pending").style.background = "#f3e7d3";
          document.getElementById("tab-pending").style.color = "#8a6a35";
          document.getElementById("tab-pending").style.border = "1px solid #c8a96b";
          document.getElementById("tab-dashboard").style.background = "#f3e7d3";
          document.getElementById("tab-dashboard").style.color = "#8a6a35";
          document.getElementById("tab-dashboard").style.border = "1px solid #c8a96b";
          loadAllMembers();
        };
        document.getElementById("tab-dashboard").onclick = function() {
          document.getElementById("tab-dashboard").style.background = "#c9a86a";
          document.getElementById("tab-dashboard").style.color = "white";
          document.getElementById("tab-dashboard").style.border = "none";
          document.getElementById("tab-pending").style.background = "#f3e7d3";
          document.getElementById("tab-pending").style.color = "#8a6a35";
          document.getElementById("tab-pending").style.border = "1px solid #c8a96b";
          document.getElementById("tab-all").style.background = "#f3e7d3";
      ...
      function openInfoPanel() {
        ...
        panel.appendChild(box);
        document.body.appendChild(panel);
        document.getElementById("close-info").onclick = function() { panel.remove(); };
  ...
  document.getElementById("avatar-upload").onchange = async function() {
    ...
    try {
      ...
      document.getElementById("avatar-msg").innerText = "Erreur lors de l upload.";
        ...
        document.getElementById("save-info").onclick = function() {
          ...
          db.collection("users").doc(u.uid).update({
            ...
            dateNaissance: document.getElementById("field-ddn").value
          }).then(function() {
      ...
      function openNotesPanel() {
        ...
        panel.appendChild(box);
        document.body.appendChild(panel);
        document.getElementById("close-notes").onclick = function() { panel.remove(); };
        const uid = auth.currentUser.uid;
        db.collection("users").doc(uid).get().then(function(snap) {
          const d = snap.data();
          document.getElementById("notes-content").value = d.notesPerso || "";
        });
        document.getElementById("save-notes").onclick = function() {
          const content = document.getElementById("notes-content").value;
          db.collection("users").doc(uid).update({ notesPerso: content }).then(function() {
      ...
      function openCarnetPanel() {
        ...
        panel.appendChild(box);
        document.body.appendChild(panel);
        document.getElementById("close-carnet").onclick = function() { panel.remove(); };
        const uid = auth.currentUser.uid;
          ...
          db.collection("users").doc(uid).collection("clientes").orderBy("nom").get().then(function(snapshot) {
            ...
            snapshot.forEach(function(docSnap) {
              ...
              document.getElementById("edit-" + cid).onclick = function() {
                ...
                document.getElementById("carnet-form").scrollIntoView({ behavior: "smooth" });
              };
              document.getElementById("del-" + cid).onclick = function() {
                if (confirm("Supprimer " + c.prenom + " " + c.nom + " ?")) {
                  db.collection("users").doc(uid).collection("clientes").doc(cid).delete().then(function() { loadClientes(); });
                }
              };
            });
          });
        ...
        document.getElementById("cancel-cliente").onclick = function() {
          document.getElementById("carnet-form").style.display = "none";
        };
        document.getElementById("save-cliente").onclick = function() {
          ...
          if (editId) {
            db.collection("users").doc(uid).collection("clientes").doc(editId).update(clienteData).then(function() {
              document.getElementById("carnet-form").style.display = "none";
              loadClientes();
            });
          } else {
            db.collection("users").doc(uid).collection("clientes").add(clienteData).then(function() {
              document.getElementById("carnet-form").style.display = "none";
              loadClientes();
            });
          }
      ...
      function openCommandesPanel() {
        ...
        function loadCommandes() {
          ...
          db.collection("users").doc(uid).collection("commandes").orderBy("date", "desc").get().then(function(snapshot) {
            ...
            snapshot.forEach(function(docSnap) {
              ...
              list.appendChild(card);
              document.getElementById("edit-cmd-" + cid).onclick = function() {
                document.getElementById("commande-form").style.display = "block";
                ...
                document.getElementById("cmd-montant").value = c.montant || "";
                document.getElementById("cmd-edit-id").value = cid;
                document.getElementById("commande-form").scrollIntoView({ behavior: "smooth" });
              };
              document.getElementById("del-cmd-" + cid).onclick = function() {
                if (confirm("Supprimer cette commande ?")) {
                  db.collection("users").doc(uid).collection("commandes").doc(cid).delete().then(function() { loadCommandes(); });
                }
              };
            });
          });
        }
        document.getElementById("add-commande-btn").onclick = function() {
          document.getElementById("commande-form").style.display = "block";
        ...
        document.getElementById("cancel-commande").onclick = function() {
        ...
        document.getElementById("save-commande").onclick = function() {
          ...
          if (editId) {
            db.collection("users").doc(uid).collection("commandes").doc(editId).update(cmdData).then(function() {
              document.getElementById("commande-form").style.display = "none";
              loadCommandes();
            });
          } else {
            db.collection("users").doc(uid).collection("commandes").add(cmdData).then(function() {
              document.getElementById("commande-form").style.display = "none";
              loadCommandes();
            });
          }
      ...
      function openOutilsPanel() {
        ...
        document.getElementById("outil-tab-suivi").onclick = function() { panel.remove(); openSuiviObjectifPanel(); };
        document.getElementById("outil-tab-checklist").onclick = function() { panel.remove(); openChecklistPanel(); };document.getElementById("outil-tab-tunnel").onclick = function() { panel.remove(); openTunnelVentePanel(); };
        document.getElementById("calc-btn").onclick = function() {
          ...
          const taux = ca >= 100 ? 30 : 20;
          const commission = ca * taux / 100;
...
function openTunnelVentePanel() {
  ...
  document.getElementById("tunnel-btn-vente").onclick = function() {
    document.getElementById("tunnel-vente-content").style.display = "block";
    document.getElementById("tunnel-recrutement-content").style.display = "none";
    document.getElementById("tunnel-btn-vente").style.background = "#c9a86a";
    document.getElementById("tunnel-btn-vente").style.color = "white";
    document.getElementById("tunnel-btn-vente").style.border = "none";
    document.getElementById("tunnel-btn-recrutement").style.background = "#f3e7d3";
    document.getElementById("tunnel-btn-recrutement").style.color = "#8a6a35";
    document.getElementById("tunnel-btn-recrutement").style.border = "1px solid #c8a96b";
  };
  document.getElementById("tunnel-btn-recrutement").onclick = function() {
    document.getElementById("tunnel-recrutement-content").style.display = "block";
    document.getElementById("tunnel-vente-content").style.display = "none";
    document.getElementById("tunnel-btn-recrutement").style.background = "#c9a86a";
    document.getElementById("tunnel-btn-recrutement").style.color = "white";
    document.getElementById("tunnel-btn-recrutement").style.border = "none";
    document.getElementById("tunnel-btn-vente").style.background = "#f3e7d3";
    document.getElementById("tunnel-btn-vente").style.color = "#8a6a35";
    document.getElementById("tunnel-btn-vente").style.border = "1px solid #c8a96b";
  };
  var messagesVente = {
  ...
  function afficherMessages(msgs, containerId) {
    ...
    msgs.forEach(function(m) {
      html += "<div style='background:white;border-radius:12px;padding:16px;margin-bottom:12px;border:1px solid #e8d4b0;'>";
      html += "<div style='color:#c9a86a;font-size:12px;font-weight:bold;margin-bottom:8px;'>" + m.etape + "</div>";
      html += "<div style='color:#3a3a3a;font-size:13px;line-height:1.6;margin-bottom:10px;'>" + m.msg + "</div>";
      html += "<button onclick='var t=this;navigator.clipboard.writeText(this.previousElementSibling.innerText).then(function(){t.innerText=\"Copie !\";setTimeout(function(){t.innerText=\"Copier\"},2000)});' style='background:#f3e7d3;color:#8a6a35;border:1px solid #c8a96b;padding:6px 14px;border-radius:8px;cursor:pointer;font-size:12px;font-weight:bold;'>Copier</button>";
      html += "</div>";
    });
    html += "</div>";
  ...
  document.querySelectorAll(".prob-btn").forEach(function(btn) {
    btn.onclick = function() {
      document.querySelectorAll(".prob-btn").forEach(function(b) { b.style.background = "white"; b.style.border = "1px solid #e8d4b0"; b.style.color = "#3a3a3a"; });
      btn.style.background = "#c9a86a"; btn.style.color = "white"; btn.style.border = "none";
      afficherMessages(messagesVente[btn.getAttribute("data-prob")], "tunnel-messages");
    };
  });
  document.querySelectorAll(".recr-btn").forEach(function(btn) {
    btn.onclick = function() {
      document.querySelectorAll(".recr-btn").forEach(function(b) { b.style.background = "white"; b.style.border = "1px solid #e8d4b0"; b.style.color = "#3a3a3a"; });
      btn.style.background = "#c9a86a"; btn.style.color = "white"; btn.style.border = "none";
      afficherMessages(messagesRecrutement[btn.getAttribute("data-recr")], "recrutement-messages");
    };
  });
}
      ...
      function openChecklistPanel() {
        ...
        function afficherTaches(taches, cochees) {
          ...
          taches.forEach(function(tache, index) {
            const estCochee = cochees.indexOf(tache) !== -1;
            const estPerso = tachesBase.indexOf(tache) === -1;
            ...
            item.onclick = function(e) {
              ...
              db.collection("users").doc(uid).update({ checklistDate: today, checklistCochees: newCochees, checklistTaches: taches }).then(function() {
                afficherTaches(taches, newCochees);
                mettreAJourProgression(taches, newCochees);
              });
            };
            if (estPerso) {
              ...
              if (delBtn) {
                delBtn.onclick = function(e) {
                  e.stopPropagation();
                  var newTaches = taches.filter(function(t) { return t !== tache; });
                  var newCochees = cochees.filter(function(c) { return c !== tache; });
                  db.collection("users").doc(uid).update({ checklistDate: today, checklistCochees: newCochees, checklistTaches: newTaches }).then(function() {
                    afficherTaches(newTaches, newCochees);
                    mettreAJourProgression(newTaches, newCochees);
                  });
                };
              }
            }
        ...
        db.collection("users").doc(uid).get().then(function(snap) {
          ...
          if (d.checklistDate === today) {
            ...
            db.collection("users").doc(uid).update({ checklistDate: today, checklistCochees: [], checklistTaches: tachesBase });
          }
          afficherTaches(taches, cochees);
          mettreAJourProgression(taches, cochees);
        });
        document.getElementById("add-task-btn").onclick = function() {
          ...
          db.collection("users").doc(uid).get().then(function(snap) {
            const d = snap.data();
            var taches = d.checklistTaches || tachesBase.slice();
            var cochees = d.checklistCochees || [];
            if (taches.indexOf(newTask) !== -1) { alert("Cette tache existe deja."); return; }
            taches.push(newTask);
            db.collection("users").doc(uid).update({ checklistDate: today, checklistCochees: cochees, checklistTaches: taches }).then(function() {
              input.value = "";
              afficherTaches(taches, cochees);
              mettreAJourProgression(taches, cochees);
            });
      ...
      function openSuiviObjectifPanel() {
        ...
        panel.appendChild(box);
        document.body.appendChild(panel);
        document.getElementById("close-suivi").onclick = function() { panel.remove(); openOutilsPanel(); };
        const uid = auth.currentUser.uid;
        function afficherProgression(objectif, realise) {
          ...
          const taux = realise >= 100 ? 30 : 20;
          const commission = realise * taux / 100;
          ...
          document.getElementById("suivi-result").style.display = "block";
        }
        db.collection("users").doc(uid).get().then(function(snap) {
          const d = snap.data();
          if (d.suiviObjectif) {
            document.getElementById("suivi-objectif").value = d.suiviObjectif || "";
            document.getElementById("suivi-realise").value = d.suiviRealise || "";
            afficherProgression(d.suiviObjectif, d.suiviRealise || 0);
          }
        });
        document.getElementById("save-suivi").onclick = function() {
          const objectif = parseFloat(document.getElementById("suivi-objectif").value);
          const realise = parseFloat(document.getElementById("suivi-realise").value) || 0;
          if (!objectif || objectif <= 0) { alert("Saisis un objectif valide."); return; }
          db.collection("users").doc(uid).update({ suiviObjectif: objectif, suiviRealise: realise }).then(function() {
            document.getElementById("suivi-save-msg").innerText = "Sauvegarde !";
            setTimeout(function() { document.getElementById("suivi-save-msg").innerText = ""; }, 3000);
            afficherProgression(objectif, realise);
          });
        };
        document.getElementById("reset-suivi").onclick = function() {
          if (confirm("Remettre a zero pour un nouveau mois ?")) {
            db.collection("users").doc(uid).update({ suiviObjectif: 0, suiviRealise: 0 }).then(function() {
              document.getElementById("suivi-objectif").value = "";
              document.getElementById("suivi-realise").value = "";
              document.getElementById("suivi-result").style.display = "none";
            });
          }
        };
      }
      ...
      function openSimulateurObjectifPanel() {
        ...
        document.getElementById("obj-sans-equipe").onclick = function() {
          avecEquipe = false;
          document.getElementById("obj-sans-equipe").style.background = "#c9a86a";
          document.getElementById("obj-sans-equipe").style.color = "white";
          document.getElementById("obj-sans-equipe").style.border = "none";
          document.getElementById("obj-avec-equipe").style.background = "#f3e7d3";
          document.getElementById("obj-avec-equipe").style.color = "#8a6a35";
          document.getElementById("obj-avec-equipe").style.border = "1px solid #c8a96b";
          document.getElementById("equipe-section").style.display = "none";
        };
        document.getElementById("obj-avec-equipe").onclick = function() {
          avecEquipe = true;
          document.getElementById("obj-avec-equipe").style.background = "#c9a86a";
          document.getElementById("obj-avec-equipe").style.color = "white";
          document.getElementById("obj-avec-equipe").style.border = "none";
          document.getElementById("obj-sans-equipe").style.background = "#f3e7d3";
          document.getElementById("obj-sans-equipe").style.color = "#8a6a35";
          document.getElementById("obj-sans-equipe").style.border = "1px solid #c8a96b";
          document.getElementById("equipe-section").style.display = "block";
        };
        document.getElementById("calc-objectif-btn").onclick = function() {
          const objectif = parseFloat(document.getElementById("obj-revenu").value);
          if (!objectif || objectif <= 0) { alert("Saisis un objectif valide."); return; }
          ...
          if (revenusVenteNecessaires <= 0) {
            document.getElementById("objectif-result").innerHTML = "<div style='background:#c9a86a;border-radius:12px;padding:20px;text-align:center;'><div style='color:white;font-size:16px;font-weight:bold;'>Tes revenus equipe couvrent deja ton objectif !</div></div>";
            document.getElementById("objectif-result").style.display = "block";
            return;
          }
          var caFinal = revenusVenteNecessaires / 0.30;
          var taux = 30;
          ...
          if (avecEquipe && revenusEquipe > 0) {
            resultHTML += "<div style='color:#888;font-size:13px;margin-bottom:8px;'>Revenus equipe : " + revenusEquipe + " euros</div>";
            resultHTML += "<div style='color:#888;font-size:13px;margin-bottom:16px;'>Revenus vente necessaires : " + revenusVenteNecessaires.toFixed(2) + " euros</div>";
          }
          resultHTML += "<div style='color:#3a3a3a;font-size:14px;margin-bottom:4px;'>Taux : <strong>" + taux + "%</strong></div></div>";
          resultHTML += "<div style='background:#c9a86a;border-radius:12px;padding:20px;text-align:center;'><div style='color:white;font-size:14px;margin-bottom:4px;'>Chiffre d affaires a realiser</div><div style='color:white;font-size:32px;font-weight:bold;'>" + caFinal.toFixed(2) + " euros</div></div>";
          document.getElementById("objectif-result").innerHTML = resultHTML;
          document.getElementById("objectif-result").style.display = "block";
        };
      }
      ...
      function openCalculEquipePanel() {
        ...
        var brancheCount = 0;
        function getPalier(vgp) {
          if (vgp >= 5000) return 17;
          if (vgp >= 3000) return 14;
          if (vgp >= 2000) return 12;
          if (vgp >= 1500) return 10;
          if (vgp >= 1000) return 8;
          if (vgp >= 500) return 6;
          if (vgp >= 250) return 4;
          if (vgp >= 100) return 2;
          return 0;
        ...
        document.getElementById("add-branche-btn").onclick = function() {
          ...
          branche.id = "branche-" + brancheCount;
        ...
        document.getElementById("calc-equipe-btn").onclick = function() {
          ...
          var totalVGPEquipe = vgpPerso;
          var branches = [];
          var valid = true;
          for (var i = 1; i <= brancheCount; i++) {
            const el = document.getElementById("branche-" + i);
            if (!el) continue;
          ...
          branches.forEach(function(b) {
            const palierBranche = getPalier(b.vgp);
            const differentiel = Math.max(0, monPalierTotal - palierBranche);
            const gain = b.vgp * differentiel / 100;
            totalDifferentiel += gain;
          ...
          document.getElementById("equipe-result").innerHTML = detailHTML;
          document.getElementById("equipe-result").style.display = "block";
        };
      }
    }
  });
...
if (document.readyState === "complete") {
  setTimeout(initBeautyAddictLogin, 1500);
} else {
  window.addEventListener("load", function() {
    setTimeout(initBeautyAddictLogin, 2000);
  });
}
