...
(function() {
  ...
  function ouvrirEditionCustom(index) {
    ...
    db.collection("boutiques").doc(user.uid).get().then(function(snap) {
      ...
      // Prix promo
      var promoLbl2 = document.createElement("p"); promoLbl2.style.cssText = "color:#8b735d;font-size:12px;font-weight:bold;margin:0 0 4px;"; promoLbl2.textContent = "🏷️ Prix promo (optionnel)"; box.appendChild(promoLbl2);
      var promoInp2 = document.createElement("input"); promoInp2.type = "number"; promoInp2.id = "em-promo"; promoInp2.step = "0.01"; promoInp2.placeholder = "Ex: 12.50";
      var existingKey = "custom_" + index + "_" + (prod.nom || "").replace(/[^a-zA-Z0-9]/g, "_").slice(0, 20);
      var existingPromoVal = (snap.data().prixPromo && snap.data().prixPromo[existingKey]) ? snap.data().prixPromo[existingKey] : "";
      promoInp2.value = existingPromoVal;
      promoInp2.style.cssText = "width:100%;padding:11px;border:1px solid #e74c3c;border-radius:10px;font-size:13px;box-sizing:border-box;margin-bottom:10px;";
      promoInp2.addEventListener("touchstart", function(e){e.stopPropagation();}, {passive:true}); box.appendChild(promoInp2);
      ...
      saveBtn.onclick = function() {
        ...
        db.collection("boutiques").doc(user.uid).get().then(function(freshSnap) {
          ...
          db.collection("boutiques").doc(user.uid).set({ produitsCustom: prods }, { merge: true }).then(function() {
            // Sauvegarder le prix promo
            var promoVal = parseFloat(document.getElementById("em-promo") ? document.getElementById("em-promo").value : 0);
            var promoKey = "custom_" + index + "_" + (nom || "").replace(/[^a-zA-Z0-9]/g, "_").slice(0, 20);
            var promoUpd = {};
            if (promoVal > 0) { promoUpd["prixPromo." + promoKey] = promoVal; }
            else { promoUpd["prixPromo." + promoKey] = firebase.firestore.FieldValue.delete(); }
            db.collection("boutiques").doc(user.uid).update(promoUpd).catch(function(){});
            modal.remove();
